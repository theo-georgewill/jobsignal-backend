...
import { getLogoUrl, normalizeCompanyName } from '../common/utils/company.util';
...
export class CompanyService {
  ...
  async update(id: string, data: UpdateCompanyDto) {
    const company = await this.prisma.company.update({
      ...
      data: {
        ...data,
        name: data.name ? normalizeCompanyName(data.name) : undefined,
      },
    });
  ...
  async bulkImport(rows: CreateCompanyDto[]) {
    let created = 0;
