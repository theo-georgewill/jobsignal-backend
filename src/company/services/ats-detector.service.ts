...
export class AtsDetectorService {
  private readonly logger =
    new Logger(
      AtsDetectorService.name,
    );

  async detect(
    careersUrl?: string | null,
  ): Promise<AtsProvider | null> {
    if (!careersUrl) {
      return null;
    }
    ...
    try {
      this.logger.log(
        `[ATS] Detecting ATS for ${careersUrl}`,
      );
      ...
      if (urlDetection) {
        this.logger.log(
          `[ATS] URL detection matched ${urlDetection}`,
        );
      ...
      if (htmlDetection) {
        this.logger.log(
          `[ATS] HTML detection matched ${htmlDetection}`,
        );
      ...
      if (scriptDetection) {
        this.logger.log(
          `[ATS] Script detection matched ${scriptDetection}`,
        );
